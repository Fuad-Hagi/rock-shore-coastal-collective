...
async function createShopifyCart(item: CartItem) {
  ...
  if (result?.cartCreate?.userErrors?.length > 0) {
    console.error("Cart creation failed:", result.cartCreate.userErrors);
    return null;
  }
...
async function updateShopifyCartLine(
  ...
  const userErrors = (data?.data as any)?.cartLinesUpdate?.userErrors || [];
  if (isCartNotFoundError(userErrors)) return { success: false, cartNotFound: true };
  if (userErrors.length > 0) return { success: false };
  return { success: true };
}
...
async function removeLineFromShopifyCart(cartId: string, lineId: string) {
  ...
  const userErrors = (data?.data as any)?.cartLinesRemove?.userErrors || [];
  if (isCartNotFoundError(userErrors)) return { success: false, cartNotFound: true };
  if (userErrors.length > 0) return { success: false };
  return { success: true };
}
